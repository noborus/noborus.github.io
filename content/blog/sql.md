...
これだけでは、条件式になっていません。ここから2のテーブルが「式」だった場合にしてみます。
...
この条件式がtrueの場合のみ残すのがテーブルの条件式です。そして、条件式は明示しない限り、結果の列には入りません。
| A | B |
|---|---|
| 2 | 4 |

AテーブルのA=2の行を抽出するだけの簡単なことを、まわりくどく説明しただけですが、そのまま延長して考えることができます。
