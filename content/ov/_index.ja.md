...
ページャーは大きく分けて、テキストファイルや標準入力から受け取って表示するパターンと、コマンドの内部から呼び出されて自動で起動するものパターンがあります。
テキストファイルや標準入力から受け取って表示する場合は、コマンドを指定するときにオプションを指定起動します。
コマンドの内部から呼び出される場合は、そのコマンドの設定ファイルや環境変数を設定することで、ovをページャーとして使用できます。

例えば、`git diff`をgitのページャーの設定として指定して起動するか`git diff | ov`のようにコマンドを指定して起動するかでは、`git`コマンドが内部での動作を変えていて、表示が異なります。
